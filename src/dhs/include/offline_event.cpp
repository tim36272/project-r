...
#define MENU_TWO_PERSON_SEARCH 3
#define MENU_TWO_PERSON_STEAL 4
#define MENU_TWO_PERSON_EXCHANGE 5
...
uint MenuInitial(const utility::ColorSingleListType& bag_colors,const utility::ColorPairListType& people_colors) {
	cv::Mat menu_image(cv::Size(kMenuWidth,kMenuItemHeight*6),CV_8UC3,cv::Scalar(0,0,0));
	...
	top_left.y+= kMenuItemHeight;
	MakeButton(&menu_image,"Two people meeting",top_left,cv::Scalar(0,0,0));
	top_left.y+= kMenuItemHeight;
...
uint MenuGetBag(const utility::ColorSingleListType& bag_colors) {
	...
	top_left.y+= kMenuItemHeight;
	MakeButton(&menu_image,"",top_left,cv::Scalar(0,0,0));
	top_left.y+= kMenuItemHeight;
