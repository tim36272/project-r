...
namespace utility {
	...
	bool AreVerticallyAlignedOriented(const Region& first, const Region& second) {
		...
		if(abs(upper_x_center-lower_x_center)<average_width/2) {
			...
			//enfore the top-over-bottom constraint
				if(bottom_upper_y-top_lower_y < 30) {
					//check if heights and widths are compatible
						//the second height differs by no more than 50% of each height
	...
	void NewPerson(const cv::Mat& depth_raw, const Region& upper, const Region& lower,const utility::Pair_<cv::Scalar> color, int frame_number, PersonDescriptor* person) {
	...
	void NewBag(const cv::Mat& depth_raw,const Region& region,const cv::Scalar color, int frame_number, int owner, BagDescriptor* bag) {
		bag->depth_position = GetMeanDepth(depth_raw,region);
		std::cout<<"TS_3CP1"<<std::endl;
		bag->last_location = boundingRect(region);
		std::cout<<"TS_3CP2"<<std::endl;
...
//			imshow("masked with depth",*out);
	...
	void UpdatePerson(const cv::Mat& depth_raw, int frame_number, const Region& upper,const Region& lower, PersonDescriptor* person) {
		person->depth_position = GetMeanDepth(depth_raw,upper,lower);
		person->last_upper_location = cv::boundingRect(upper);
		person->last_lower_location = cv::boundingRect(lower);
	...
	void UpdateBag(const cv::Mat& depth_raw, int frame_number, const Region& region, BagDescriptor* bag) {
		bag->depth_position = GetMeanDepth(depth_raw,region);
		bag->last_location = boundingRect(region);
	...
	Pair TryToUpdatePerson(const cv::Mat& depth_raw, int frame_number, const RegionList& upper_regions, const RegionList& lower_regions, PersonDescriptor* person) {
		...
		for(uint upper_candidate_index=0; upper_candidate_index<upper_candidates.size(); upper_candidate_index++) {
			for(uint lower_candidate_index=0; lower_candidate_index<lower_candidates.size(); lower_candidate_index++) {
				if(utility::AreVerticallyAlignedOriented(upper_regions[upper_candidate_index],lower_regions[lower_candidate_index])) {
					full_candidates.push_back(utility::Pair(upper_candidate_index,lower_candidate_index));
				}
			}
		}
		if(full_candidates.size()==1) {
			UpdatePerson(depth_raw,frame_number, upper_regions[full_candidates[0].values[0]],lower_regions[full_candidates[0].values[1]],person);
			...
			UpdatePerson(depth_raw,frame_number, upper_regions[full_candidates[max_at][0]],lower_regions[full_candidates[max_at][1]],person);
			return full_candidates[max_at];
		}
		return utility::Pair();
	}
	...
	int TryToUpdateBag(const cv::Mat& depth_raw, int frame_number, const RegionList& regions, const cv::Rect owner_location, BagDescriptor* bag) {
		...
		if(candidates.size()==1) {
			UpdateBag(depth_raw,frame_number, regions[candidates[0]],bag);
			return candidates[0];
		}
			...
			UpdateBag(depth_raw,frame_number, regions[candidates[max_at]],bag);
			return candidates[max_at];
		}
		return -1;
	}
